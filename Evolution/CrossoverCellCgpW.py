...
class CrossoverCellCgpW(Crossover):
    """
    ...
    def croosover_base(self, X, problem, index):
        """
        ...
        for k in range(tem):
            for p in range(n_matings):
                ...
                X_ori[k, p, 0].individual[index] = GenCgpW(
    ...
    def _do(self, problem, X, **kwargs):
        ...
        for i in range(X[0, 0, 0].normal):
            X_temp = self.croosover_base(X_temp, problem, i)
...
#
#         X_ori = np.full(X_copy.shape, None)
#         tem, n_matings, n_var = c.shape
#         shapeX = X_copy[0, 0, 0].individual[index].realgene.shape
#
#         for k in range(tem):
#             for p in range(n_matings):
#                 X_ori[k, p, 0] = copy.deepcopy(X_copy[k, p, 0])
#                 X_ori[k, p, 0].individual[index]  = GenCgpW(X_copy[k, p, 0].conf_net[index], np.copy(c[k, p].reshape(shapeX)),
#                                          weights=X_copy[k, p, 0].individual[index].weightcolums)
#                 X_ori[k, p, 0].individual[index].to_int_cgp()
#
#         return X_ori
